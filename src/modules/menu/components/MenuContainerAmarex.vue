...
export default {
  ...
  computed: {
    ...mapGetters(["menuFromConfig", "isMobile", "uiStyle"]),
    ...
    ...mapGetters("Modules/ProjectStarter", ["projectTitle"]),
    /**
  ...
  created() {
    this.mergeMenuState({
      menu: this.menuFromConfig(this.side),
      side: this.side,
    });
    if (this.isMobile) {
      this.collapseMenues();
  ...
  <div
    ...
    :style="{
      width: expanded
        ? side === 'mainMenu'
          ? this.mainMenuWidth
          : this.secondaryMenuWidth
        : '0',
      padding: expanded ? '' : '0px',
      minWidth: expanded
        ? side === 'mainMenu'
          ? `${this.mainMenuWidth}px`
          : `${this.secondaryMenuWidth}px`
        : '0px',
      maxWidth: expanded ? `${this.mainMenuWidth}px` : '0px',
    }"
  ...
  >
    ...
    >
      ...
      <div
        ...
        :class="{
          'mp-menu-subHeader-collapsed':
            (!mainExpanded && side === 'mainMenu') ||
            (!secondaryExpanded && side === 'secondaryMenu'),
        }"
      >
        <MenuContainerBodyRootLogo v-bind="titleBySide(side)" />
        <SearchBar
    ...
    <div
      v-if="side === 'mainMenu' && !!projectTitle"
      class="project-info-container mb-3"
    >
